...
- [ ] 1. **Опция `lattice.pi-acp-daemon.defaultCwd`** (`types.str`, `mkDefault userHome`) в
      ...
      `userHome` напрямую.
- [ ] 2. **Контракт-тест** в [`tests/pi-acp-daemon.nix`](../../tests/pi-acp-daemon.nix):
...
- [ ] 3. **Workspace на ноде**: `/var/lib/lattice-workspace` (mode 0700, root) + запись в
      `environment.persistence."/persist".directories`
      [`nodes/mytecor-homelab/config.nix`](../../nodes/mytecor-homelab/config.nix) — без impermanence
      checkout переживёт только до reboot.
- [ ] 4. **Идемпотентный one-shot `lattice-workspace-init`**: клон RID Lattice из локального seed
...
- [ ] 5. **Node config**: `lattice.pi-acp-daemon.defaultCwd = "/var/lib/lattice-workspace/lattice";`
