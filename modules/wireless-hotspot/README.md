...
> **Статус: отключено на ноде `mytecor-homelab`.** Модуль оставлен в репозитории
> как реализация и документация подхода, но рабочий hotspot на текущем железе
> невозможен. См. [Почему hotspot отключён](#почему-hotspot-отключён) ниже.
...
## Почему hotspot отключён

Точка доступа на `rtw88_8822ce` работает **только при STA на 2.4 GHz**
(подтверждено живым тестом). Как только STA уходит на **5 GHz**, concurrent
AP ломается во всех проверенных режимах:

| Режим AP (5 GHz) | Симптом |
|---|---|
| VHT20 (`ieee80211ac` + 20 MHz) | hostapd поднимается, но data path рвётся: клиент ассоциируется (EAPOL OK), не получает DHCP, отваливается через ~18 с; в dmesg — шквал `rtw88_8822ce PCIe Bus Error (RxErr)` |
| VHT80 (`vht_oper_chwidth=1`) | hostapd падает при старте с активным STA |
| HT20 (802.11n-only) | клиент вообще не ассоциируется |

Причина — единый радио-чип (`#channels <= 1`): STA и AP обязаны делить один
канал. Когда гостевой роутер (к которому подключена STA) вещает **только на
5 GHz**, hotspot вынужден работать на 5 GHz и нестабилен на этом железе
независимо от конфигурации (Firewall/DHCP/NAT настроены корректно и проверены:
проблема не в конфиге, а в драйвере/чипе).

Попытка перевести STA на 2.4 GHz (принудительный `band = bg`) не помогла:
у гостевого роутера нет надёжной 2.4-сети, и принудительный переход STA на
2.4 оборвал связь с нодой. Рабочий вариант требует **отдельного Wi-Fi-радио**
для AP (например USB-адаптер 2.4 GHz), которого на ноде нет.

Поэтому `lattice.hotspot.enable = false` на `mytecor-homelab`, и модуль
оставлен как референс-реализация подхода.
