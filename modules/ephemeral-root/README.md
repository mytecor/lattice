...
только те идентичности сервисов, которым требуется стабильность. Reticulum identity может не
сохраняться до публикации постоянного destination; после публикации её потеря меняет адрес сервиса,
поэтому ключ должен находиться в persistence или восстанавливаться через `agenix`. Эти решения
являются частью конфигурации ноды, а не универсального модуля.
