...
Сейчас у homelab не включены Reticulum/rnsh и Radicle. Централизованного механизма исключения
...
Когда rnsh будет включён в F3, отозванные initiator hashes удаляются из `lattice.rnsh.allowed`
...
топология, проверка активных сессий и способ отзыва общей Network Identity, если она появится,
должны быть проверены при реализации F3. Изоляция транспорта выполняется на реально управляемых
точках входа, а не вымышленной командой «ban node».
