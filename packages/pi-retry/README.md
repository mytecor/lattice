...
С сентября 2026 llm-gateway дополнительно отдаёт в `event: error`
машинно-читаемые поля (`retryable`, `partial`, `status_code`, `request_id`),
а текст `message` (`"upstream stream failed"`) остаётся стабильным, так что
паттерн-матчинг продолжает работать. Полевой матчинг (`retryable: true`,
`partial: true`) можно добавить в сам форк как более надёжную альтернативу
тексту; также обрыв стрима теперь сам понижает здоровье провайдера в шлюзе
(cooldown/health), так что ретрай чаще уходит уже не на сломанного провайдера.
