...
func continueRuleWait(route string, idle time.Duration, reshare string, retries int, wait time.Duration) Rule {
	r := continueRuleRetries(route, idle, reshare, retries).(*ContinueRule)
	r.Wait = Duration{Duration: wait}
	return r
}
