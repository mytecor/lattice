...
func TestExpositionRendersFamilies(t *testing.T) {
	...
	m.ObserveCooldownUntil("a", "native-model", time.Unix(1_700_000_000, 0))
...
func TestInFlightAcrossConcurrentBranches(t *testing.T) {
	...
	if !strings.Contains(body, `llm_attempts_total{provider="a",native_model="native-model",error_type=""} 16`) {
...
func TestCooldownUntilMetric(t *testing.T) {
	...
	m.ObserveCooldownUntil("b", "native-model", time.Unix(1_700_000_007, 0))
	m.ObserveCooldownUntil("c", "native-model", time.Time{}) // clear: expired windows must not linger
	body := scrape(t, m)

	if !strings.Contains(body, `llm_cooldown_until_seconds{provider="a",native_model="native-model"} 1.7000001e+09`) {
		t.Errorf("cooldown deadline for a missing or wrong:\n%s", body)
	}
	if !strings.Contains(body, `llm_cooldown_until_seconds{provider="b",native_model="native-model"} 1.700000007e+09`) {
		t.Errorf("cooldown deadline for b missing or wrong:\n%s", body)
	}
	// A cleared pair must be fully absent from the family: a literal zero
	// deadline would be indistinguishable from a just-expired window.
	if strings.Contains(body, `llm_cooldown_until_seconds{provider="c",native_model="native-model"}`) {
		t.Errorf("cleared cooldown must not linger as a stale entry:\n%s", body)
	}
	// Extension overwrites in place: one series per pair, latest value wins.
	m.ObserveCooldownUntil("a", "native-model", time.Unix(1_700_000_005, 0))
	body = scrape(t, m)
	if strings.Contains(body, `llm_cooldown_until_seconds{provider="a",native_model="native-model"} 1.7000001e+09`) {
