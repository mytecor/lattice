...
func TestCooldownUntilMetric(t *testing.T) {
	...
	if !strings.Contains(body, `llm_cooldown_until_seconds{provider="a",model="native-model"} 1.7000001e+09`) {
		t.Errorf("cooldown deadline for a missing or wrong:\n%s", body)
	}
	if !strings.Contains(body, `llm_cooldown_until_seconds{provider="b",model="native-model"} 1.700000007e+09`) {
	...
	if strings.Contains(body, `llm_cooldown_until_seconds{provider="c",model="native-model"}`) {
