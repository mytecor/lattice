...
ключи и данные, стабильность которых является частью её контракта. В частности, Reticulum identity
первой ноды может быть пересоздана при миграции и не входит в обязательный набор persistence.
...
recipient. Общая модель идентичности для следующих нод остаётся задачей
...
ключа ноды и recovery-ключа администратора и проверяются сборкой. Дальнейшая ротация и общая модель
идентичности остаются задачами F2.
