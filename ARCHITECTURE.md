...
## Wi-Fi-радио ноды: результат живого теста STA+AP

Физическое Wi-Fi-радио на ноде (Realtek RTL8822CE, драйвер `rtw88_8822ce`) имеет **один
радио-канал** и допускает ровно один client-линк при ровно одном AP-интерфейсе:
...
Живым тестом на ноде (зафиксирован в снятом модуле `wireless-hotspot`, реализация
**одновременного** STA+AP) установлено:

- Одновременный STA+AP **работает только при STA на 2.4 GHz**.
- Как только STA уходит на **5 GHz**, concurrent AP ломается во всех проверенных режимах
  (VHT20 рвёт data path, VHT80 валит hostapd, HT20 — клиент не ассоциируется).
- Причина — единый радио-чип (`#channels <= 1`): STA и AP обязаны делить один канал.
  Рабочий вариант с одновременностью требует **отдельного второго Wi-Fi-радио**.

Поэтому одновременный STA+AP на этом железе осознанно не используется. Поддерживаемый способ
дать ноде роль точки доступа — **переключение режима целиком** (проводной аплинк есть → Wi-Fi
работает только как AP, STA выключен; провода нет → Wi-Fi возвращается в клиентский режим),
см. [F17](roadmap/f17-wifi-hotspot-switch/README.md).
